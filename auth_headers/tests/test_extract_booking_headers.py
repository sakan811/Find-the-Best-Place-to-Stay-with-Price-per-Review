...
class TestBookingHeaderExtractor:
    ...
    def test_write_env_variables_with_headers(self, mock_dirname, mock_file, mock_env_get, mock_logger):
        ...
        mock_logger.info.assert_called_with(pytest.approx("Environment variables saved to"))
...
async def test_extract_headers_success(mock_playwright):
    ...
    # Mock page.route to set headers in the extractor
    async def mock_route_func(pattern, handler):
        extractor = BookingHeaderExtractor()
        extractor.completed = True
        extractor.headers = {"USER_AGENT": "test-user-agent"}
        # Store the extractor in the page for later access
        mock_page.extractor = extractor
    ...
    assert "USER_AGENT" in result.headers
