...
from scraper.scraper_func.graphql_func import (
    get_header,
    check_city_data,
    check_currency_data,
    check_hotel_filter_data,
    check_country_data,
)
...
class Scraper(BaseModel):
    ...
    def _log_booking_details(self):
        ...
        """
        main_logger.debug(
            f"City: {self.city} | Check-in: {self.check_in} | Check-out: {self.check_out}"
        )
        main_logger.debug(f"Currency: {self.selected_currency}")
        main_logger.debug(
            f"Adults: {self.group_adults} | Children: {self.group_children} | Rooms: {self.num_rooms}"
        )
    ...
    def scrape_graphql(self) -> pd.DataFrame:
        ...
        if not self._validate_inputs():
            main_logger.warning(
                "Error: city, check_in, check_out and selected_currency are required"
            )
    ...
    def _build_graphql_url(self) -> str:
        ...
        """
        return (
            f"https://www.booking.com/dml/graphql?ss={self.city}%2C+{self.country}&"
    ...
    def _scrape_all_pages(
        self, url: str, headers: dict, total_page_num: int
    ) -> pd.DataFrame:
        """
        ...
        return {
            ...
            "variables": {
                "input": {
                    ...
                    "childrenAges": [0],
                    "dates": {"checkin": self.check_in, "checkout": self.check_out},
                    ...
                    "pagination": {"rowsPerPage": 100, "offset": page_offset},
                    ...
                    "referrerBlock": {"blockName": "searchbox"},
    ...
    def check_info(self, data: dict) -> int:
        ...
        if total_page_num > 0:
            ...
            booking_details = BookingDetails(
                ...
                check_in=data["data"]["searchQueries"]["search"]["flexibleDatesConfig"][
                    "dateRangeCalendar"
                ]["checkin"][0],
                check_out=data["data"]["searchQueries"]["search"][
                    "flexibleDatesConfig"
                ]["dateRangeCalendar"]["checkout"][0],
                group_adults=data["data"]["searchQueries"]["search"]["searchMeta"][
                    "nbAdults"
                ],
                group_children=data["data"]["searchQueries"]["search"]["searchMeta"][
                    "nbChildren"
                ],
                num_rooms=data["data"]["searchQueries"]["search"]["searchMeta"][
                    "nbRooms"
                ],
