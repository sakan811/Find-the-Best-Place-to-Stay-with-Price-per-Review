...
class GetBookingDetailsFromDbTests(APITestCase):
    def setUp(self):
        ...
        BookingDetails.objects.create(
            check_in="2023-10-01", check_out="2023-10-05", city="City A",
            num_adults=2, num_children=1, num_rooms=1, currency="USD", only_hotel=True
        )
        BookingDetails.objects.create(
            check_in="2023-10-10", check_out="2023-10-15", city="City B",
            num_adults=3, num_children=2, num_rooms=2, currency="EUR", only_hotel=False
        )
